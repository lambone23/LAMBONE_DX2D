...
namespace yha
{
	...
	void CFmod::FnInitialize()
	{
		...
		//mCoreSystem->update();
	}//END-void CFmod::FnInitialize

	bool CFmod::FnCreateSound(const std::string& path, FMOD::Sound** sound)
	{
		if (FMOD_OK != mCoreSystem->createSound(path.c_str(), FMOD_3D, 0, sound))
			return false;

		return true;
	}//END-bool CFmod::FnCreateSound

	void CFmod::FnSoundPlay(FMOD::Sound* sound, FMOD::Channel** channel)
	{
		mCoreSystem->playSound(sound, 0, false, channel);
	}//END-void CFmod::FnSoundPlay

	void CFmod::FnSet3DListenerAttributes(const Vector3* pos, const Vector3* vel, const Vector3* forward, const Vector3* up)
	{
	...
	}//END-void CFmod::FnSet3DListenerAttributes

	void CFmod::FnRelease()
	{
		//mCoreSystem->release();
		//mCoreSystem = nullptr;
