...
namespace yha::graphics
{
	...
	private:
		...
		//Microsoft::WRL::ComPtr<ID3D11RenderTargetView> mRenderTargetView;	// 렌더타겟에 직접접근하지 않고 렌더타겟뷰를 통해서 접근
		...
		std::shared_ptr<yha::graphics::CTexture> mRenderTarget;				// 최종적으로 그려질 텍스처(도화지)
		std::shared_ptr<yha::graphics::CTexture> mDepthStencil;				// 깊이버퍼
